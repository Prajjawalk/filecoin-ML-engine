...
contract LogisticRegression {
	...
	function fit(
		...
		for (uint256 iter = 0; iter < iterations; iter++) {
			int256[] memory linearModel = new int256[](m);
			int256[] memory predictions = new int256[](m);
			int256[] memory dw = new int256[](n);
			int256 db = 0;
			...
			for (uint256 j = 0; j < n; j++) {
				weights[j] -= (learningRate * dw[j]) / int256(m);
			}
			bias -= (learningRate * db) / int256(m);
		}
	...
	function sigmoid(int256 z) internal pure returns (int256) {
		...
		return FIXED_POINT / (FIXED_POINT + expValue);
	}
